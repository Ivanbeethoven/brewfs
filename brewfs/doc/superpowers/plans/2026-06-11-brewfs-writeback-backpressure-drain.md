...
The plan is based on the current review set under `/mnt/slayerfs/brewfs/brewfs/doc/`:

- `/mnt/slayerfs/brewfs/brewfs/doc/review-writeback-writer.md`: `CommitBeforeUpload` creates metadata-visible, not-yet-uploaded slices; current soft/hard backpressure has no wait counters and can trade close tail for write p99.
- `/mnt/slayerfs/brewfs/brewfs/doc/review-perf-harness-config.md`: `direct=0` mixes kernel page cache and FUSE writeback-cache; write workloads need post-write drain accounting; all accepted optimizations must include `randrw`.
- `/mnt/slayerfs/brewfs/brewfs/doc/review-object-store-cache.md`: random write/mixed workloads are dominated by small object PUT amplification; concurrency tuning without object-count/drain metrics is easy to misread.
- `/mnt/slayerfs/brewfs/brewfs/doc/review-metadata-cache.md`: metadata batching is not the next default attempt because the earlier batch direction improved a narrow profile but failed compose `randrw`.
- `/mnt/slayerfs/brewfs/brewfs/doc/review-parallel-agents-summary.md`: implement writer pending/staging/watchdog metrics first, then improve the read/object and metadata layers.
- `/mnt/slayerfs/brewfs/brewfs/doc/perf-optimization-roadmap.md`: range-prefetch pending gate and LZ4 raw-fallback zero-copy were tested and rejected; avoid repeating them.
...
- Modify: `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`
...
- Modify: `/mnt/slayerfs/brewfs/brewfs/src/vfs/config.rs`
  - Add an opt-in backpressure policy mode, parsed from `BREWFS_WRITEBACK_BACKPRESSURE_POLICY`.
- Modify: `/mnt/slayerfs/brewfs/brewfs/docker/compose-xfstests/run_perf_in_container.sh`
  - Add report-only `PERF_FIO_POST_WRITE_DRAIN` support and artifact fields.
- Modify if needed: `/mnt/slayerfs/brewfs/brewfs/doc/perf-optimization-roadmap.md`
...
Add assertions to the existing stats render test in `/mnt/slayerfs/brewfs/brewfs/src/vfs/stats.rs`:
...
In `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`, wrap the existing waits in `wait_for_writeback_backpressure`:
...
Add tests in `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`:
...
In `/mnt/slayerfs/brewfs/brewfs/src/vfs/config.rs`, add:
...
In `/mnt/slayerfs/brewfs/brewfs/src/vfs/io/writer.rs`, add:
...
**Files:**
- Modify: `/mnt/slayerfs/brewfs/brewfs/doc/perf-optimization-roadmap.md`
...
Append a result section to `/mnt/slayerfs/brewfs/brewfs/doc/perf-optimization-roadmap.md` with:
...
```bash
cd /mnt/slayerfs/brewfs
git add brewfs/src/vfs/stats.rs brewfs/src/vfs/io/writer.rs brewfs/docker/compose-xfstests/run_perf_in_container.sh
...
```bash
cd /mnt/slayerfs/brewfs
git add brewfs/src/vfs/config.rs brewfs/src/vfs/io/writer.rs brewfs/doc/perf-optimization-roadmap.md
...
```bash
cd /mnt/slayerfs/brewfs
git diff -- brewfs/src/vfs/config.rs brewfs/src/vfs/io/writer.rs
```

Use a targeted reverse patch to remove the rejected hysteresis code, keep the useful metrics, and record the negative result in `/mnt/slayerfs/brewfs/brewfs/doc/perf-optimization-roadmap.md`.
