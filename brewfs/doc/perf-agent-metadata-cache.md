...
说明：BrewFS 代码引用来自本 worktree。JuiceFS 源码从 `/mnt/slayerfs/brewfs/brewfs/juicefs` 只读查看，因为该 worktree 中没有 `juicefs/` 目录。
...
- `MetaClient` 持有：
  - `InodeCache`：缓存 inode attr、parent、目录 children 状态和 slice metadata，见 `brewfs/src/meta/client/cache.rs`。
  - `path_cache`、`path_trie`、`inode_to_paths`：缓存 path 到 inode 的解析结果，见 `brewfs/src/meta/client/mod.rs`。
...
- Redis store 自己还有一层 `node_cache`，默认 capacity `100000`、TTL `30s`，见 `brewfs/src/meta/stores/redis/mod.rs`。因此 `MetaClient::stat_fresh` 绕过了 MetaClient inode cache，但在 Redis backend 下仍可能命中 Redis store 的本地 node cache。
- perf 脚本已经打开 open-file cache：`brewfs/tools/perf/run_perf.sh` 中 Redis profile 设置 `open_file_cache_ttl_ms: 1000`、capacity `65536`。
...
- 现有 perf 脚本：
  - `brewfs/tools/perf/run_perf.sh` metadata profile；
  - Redis meta 下 small-file create/read profile；
  - 如存在，补跑 `dirperf`/`dirstress`。

本文档自查命令：

- `rg -n "当前 BrewFS|JuiceFS 对应机制|具体差距|建议改动|预期测试场景|风险" brewfs/doc/perf-agent-metadata-cache.md`
- `git diff --check -- brewfs/doc/perf-agent-metadata-cache.md`
