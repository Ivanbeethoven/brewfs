...
选项:
  ...
  --writeback-throughput-profile
                             启用 S3 writeback 全场景吞吐 profile（4GiB read/write buffer, 12GiB memory budget, S3 max concurrency=16, writer upload_concurrency=32, writeback upload concurrency=3, pending soft/hard=1GiB/2GiB, writeback persist fsync=false, compression=lz4, fuse workers=6, fio prefill drain+remount）
...
if [[ "$WRITEBACK_THROUGHPUT_PROFILE" == true ]]; then
    ...
    export BREWFS_WRITEBACK_UPLOAD_CONCURRENCY="${BREWFS_WRITEBACK_UPLOAD_CONCURRENCY:-3}"
